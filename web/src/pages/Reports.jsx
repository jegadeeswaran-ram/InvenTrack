...
function MonthlyReport() {
  ...
  const exportExcel = () => {
    ...
    const summarySheet = XLSX.utils.aoa_to_sheet([
      ...
      ['Purchase Cost', `₹${fmt(data.summary.totalPurchaseCost)}`],
      ['Revenue', `₹${fmt(data.summary.totalRevenue)}`],
      ['Shop Revenue', `₹${fmt(data.summary.shopRevenue)}`],
      ['Truck Revenue', `₹${fmt(data.summary.truckRevenue)}`],
      ['Gross Profit', `₹${fmt(data.summary.grossProfit || data.summary.totalProfit)}`],
      ['Total Expenses', `₹${fmt(data.summary.totalExpenses)}`],
      ['Net Profit', `₹${fmt(data.summary.netProfit)}`],
    ]);
    XLSX.utils.book_append_sheet(wb, summarySheet, 'Summary');
  ...
  return (
    <div>
      ...
      {data && (
        <>
          <div className="kpi-grid" style={{ marginBottom: 20 }}>
            <div className="kpi-card"><div className="label">Revenue</div><div className="value">₹{fmt(data.summary.totalRevenue)}</div><div className="sub">🏪 ₹{fmt(data.summary.shopRevenue)} · 🚚 ₹{fmt(data.summary.truckRevenue)}</div></div>
            <div className="kpi-card"><div className="label">Gross Profit</div><div className="value" style={{ color: 'var(--success)' }}>₹{fmt(data.summary.grossProfit || data.summary.totalProfit)}</div></div>
            <div className="kpi-card" style={{ borderLeftColor: 'var(--danger)' }}><div className="label">Total Expenses</div><div className="value" style={{ color: 'var(--danger)' }}>₹{fmt(data.summary.totalExpenses)}</div></div>
            <div className="kpi-card" style={{ borderLeftColor: (data.summary.netProfit ?? data.summary.totalProfit) >= 0 ? 'var(--success)' : 'var(--danger)' }}>
              <div className="label">Net Profit</div>
              <div className="value" style={{ color: (data.summary.netProfit ?? data.summary.totalProfit) >= 0 ? 'var(--success)' : 'var(--danger)' }}>₹{fmt(data.summary.netProfit ?? data.summary.totalProfit)}</div>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            <div className="card">
