...
class _UsersScreenState extends State<UsersScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final results = await Future.wait([
        ApiService.get('/users', token: _token),
        ApiService.get('/trucks', token: _token),
      ]);
      if (mounted) setState(() {
        _users   = results[0] as List;
        _trucks  = results[1] as List;
        _loading = false;
      });
    } catch (_) {
  ...
  void _showForm({Map<String, dynamic>? user}) {
    ...
    String saleType = user?['saleType'] ?? 'SHOP';
    int? truckId = user?['truckId'] as int?;
    final formKey = GlobalKey<FormState>();
    String? errorMsg;

    final activeTrucks = _trucks.where((t) => t['isActive'] == true).toList();
                ...
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: const [
                      ...
                      DropdownMenuItem(value: 'BRANCH_MANAGER', child: Text('BRANCH_MANAGER')),
                      DropdownMenuItem(value: 'ADMIN', child: Text('ADMIN')),
                    ],
                    onChanged: (v) => setModalState(() { role = v!; if (role != 'SALES') saleType = 'SHOP'; }),
                  ),
                  if (role == 'SALES') ...[
                    const SizedBox(height: 12),
                    DropdownButtonFormField<String>(
                      value: saleType,
                      decoration: const InputDecoration(labelText: 'Sale Type', prefixIcon: Icon(Icons.sell_outlined)),
                      items: const [
                        DropdownMenuItem(value: 'SHOP', child: Text('🏪 Shop')),
                        DropdownMenuItem(value: 'TRUCK', child: Text('🚚 Truck')),
                      ],
                      onChanged: (v) => setModalState(() { saleType = v!; if (saleType != 'TRUCK') truckId = null; }),
                    ),
                    if (saleType == 'TRUCK' && activeTrucks.isNotEmpty) ...[
                      const SizedBox(height: 12),
                      DropdownButtonFormField<int?>(
                        ...
                        items: [
                          ...
                          ...activeTrucks.map<DropdownMenuItem<int?>>((t) =>
                              DropdownMenuItem<int?>(value: t['id'] as int, child: Text(t['name'] as String))),
                        ],
                        onChanged: (v) => setModalState(() => truckId = v),
                      ),
                    ],
                  ],
                  if (errorMsg != null) ...[
                    ...
                    onPressed: () async {
                      ...
                      try {
                        final body = <String, dynamic>{
                          ...
                          if (role == 'SALES') ...{
                            'saleType': saleType,
                            'truckId': saleType == 'TRUCK' ? truckId : null,
                          },
                        };
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            if (user['role'] == 'SALES')
              _detailRow('Sale Type', user['saleType'] == 'TRUCK' ? '🚚 Truck' : '🏪 Shop'),
            if (user['saleType'] == 'TRUCK' && user['truck'] != null)
              _detailRow('Truck', user['truck']['name'] as String),
            _detailRow('Status', user['isActive'] == true ? 'Active' : 'Inactive'),
  ...
  String _userSubtitle(Map<String, dynamic> u) {
    final base = '@${u['username']} · ${u['role']}';
    if (u['role'] == 'SALES') {
      final st = u['saleType'] == 'TRUCK' ? '🚚 Truck' : '🏪 Shop';
      final truck = u['truck'] as Map<String, dynamic>?;
      if (u['saleType'] == 'TRUCK' && truck != null) {
        return '$base · $st · ${truck['name']}';
      }
      return '$base · $st';
    }
    return base;
